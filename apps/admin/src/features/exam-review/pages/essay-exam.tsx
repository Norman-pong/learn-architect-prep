...
export function EssayExamPage() {
  ...
  if (phase === "report" && report) {
    return <EssayReport report={report} onBack={() => navigate({ to: "/exam/history" })} />;
  }
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col gap-4 py-4">
      {/* Header */}
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border bg-card px-4 py-3">
        <div className="flex items-center gap-3">
          <FileTextOutlined className="h-5 w-5 text-primary" />
          <h1 className="text-lg font-semibold">论文模拟考</h1>
          {selectedQuestion && (
            <span className="text-sm text-muted-foreground">{selectedQuestion.title}</span>
          )}
        </div>
        <div className="flex items-center gap-3">
          ...
          <Button variant="default" size="sm" onClick={handleFinish} className="gap-1.5">
