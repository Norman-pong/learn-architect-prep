import { useEffect, useMemo, useRef, useState } from "react";
import { useNavigate } from "react-router";
import {
  Button,
  Card,
  Checkbox,
  Collapse,
  Flex,
  Input,
  List,
  Modal,
  Progress,
  Result,
  Space,
  Statistic,
  Tag,
  Typography,
  message,
  theme,
} from "antd";
import { apiRequest } from "../api/client";

const { Title, Text, Paragraph } = Typography;
const { TextArea } = Input;

interface CaseQuestion {
  id: string;
  question: string;
  referenceAnswer: string;
  chapter: string;
  difficulty: string;
  source: string;
  year: number;
}

interface CasePaper {
  examId: string;
  questions: CaseQuestion[];
  duration: number;
  remainingTime: number;
}

interface CaseAnswer {
  answer: string;
  mermaid?: string;
}

interface Dimension {
  name: string;
  weight: number;
  score: number;
  maxScore: number;
  comment: string;
}

interface CaseReport {
  examId: string;
  score: number;
  maxTotalScore: number;
  passLine: number;
  passed: boolean;
  dimensions: Dimension[];
  overallComment: string;
  improvementSuggestions: string[];
  answers: Record<string, CaseAnswer>;
}

interface ActiveExam {
  id: string;
  examType: string;
  mode: string;
  status: string;
  duration: number;
  remainingTime: number;
  answersSnapshot: Record<string, unknown>;
  startedAt: string;
}

const CHOOSE_COUNT = 4;

function formatSeconds(total: number): string {
  const h = Math.floor(total / 3600);
  const m = Math.floor((total % 3600) / 60);
  const s = total % 60;
  return `${String(h).padStart(2, "0")}:${String(m).padStart(2, "0")}:${String(s).padStart(2, "0")}`;
}

export default function CaseExamPage() {
  const navigate = useNavigate();
  const { token } = theme.useToken();

  const [loading, setLoading] = useState(false);
  const [examId, setExamId] = useState<string | null>(null);
  const [paper, setPaper] = useState<CasePaper | null>(null);
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [answers, setAnswers] = useState<Record<string, CaseAnswer>>({});
  const [remaining, setRemaining] = useState(0);
  const [started, setStarted] = useState(false);
  const [paused, setPaused] = useState(false);
  const [report, setReport] = useState<CaseReport | null>(null);

  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // Start or resume a case exam on mount.
  useEffect(() => {
    const init = async () => {
      setLoading(true);
      try {
        const { active } = await apiRequest<{ active: ActiveExam | null }>("/api/exam/status");
        if (active?.examType === "case" && active.status === "in_progress") {
          setExamId(active.id);
          setRemaining(active.remainingTime);
          // restore selected/answers from snapshot
          const snapshot = active.answersSnapshot as {
            selected?: string[];
            answers?: Record<string, CaseAnswer>;
          };
          if (snapshot.selected) setSelected(new Set(snapshot.selected));
          if (snapshot.answers) setAnswers(snapshot.answers);
          await loadPaper(active.id);
        } else {
          const res = await apiRequest<{ id: string; questionIds: string[] }>("/api/exam/start", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ examType: "case", mode: "single" }),
          });
          setExamId(res.id);
          // load full exam record to get remainingTime
          const { active: fresh } = await apiRequest<{ active: ActiveExam | null }>(
            "/api/exam/status",
          );
          setRemaining(fresh?.remainingTime ?? 90 * 60);
          await loadPaper(res.id);
        }
      } catch (err) {
        message.error(err instanceof Error ? err.message : "启动考试失败");
      } finally {
        setLoading(false);
      }
    };
    void init().catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const loadPaper = async (id: string) => {
    try {
      const p = await apiRequest<CasePaper>(`/api/exam/case/paper?examId=${id}`);
      setPaper(p);
      setStarted(true);
    } catch (err) {
      message.error(err instanceof Error ? err.message : "加载试卷失败");
    }
  };

  // Timer countdown
  useEffect(() => {
    if (started && !paused && remaining > 0 && !report) {
      timerRef.current = setInterval(() => {
        setRemaining((prev) => {
          if (prev <= 1) {
            if (timerRef.current) clearInterval(timerRef.current);
            timerRef.current = null;
            void finishExam().catch(() => {});
            return 0;
          }
          return prev - 1;
        });
      }, 1000);
    } else if (timerRef.current) {
      clearInterval(timerRef.current);
      timerRef.current = null;
    }
    return () => {
      if (timerRef.current) {
        clearInterval(timerRef.current);
        timerRef.current = null;
      }
    };
  }, [started, paused, remaining, report]);

  const answeredCount = useMemo(() => {
    return Object.values(answers).filter((a) => a.answer.trim().length > 0).length;
  }, [answers]);

  const selectedCount = selected.size;

  const toggleQuestion = (id: string) => {
    setSelected((prev) => {
      const next = new Set(prev);
      if (next.has(id)) {
        next.delete(id);
      } else if (next.size < CHOOSE_COUNT) {
        next.add(id);
      }
      return next;
    });
  };

  const updateAnswer = (questionId: string, field: keyof CaseAnswer, value: string) => {
    setAnswers((prev) => ({
      ...prev,
      [questionId]: { ...prev[questionId], [field]: value },
    }));
  };

  const submitAnswer = async (questionId: string) => {
    if (!examId) return;
    const ans = answers[questionId] ?? { answer: "" };
    try {
      await apiRequest("/api/exam/case/submit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ examId, questionId, answer: ans.answer, mermaid: ans.mermaid }),
      });
    } catch {
      // local answer already updated; server sync failure will not block UI
    }
  };

  const pauseExam = async () => {
    if (!examId) return;
    setPaused(true);
    try {
      await apiRequest("/api/exam/pause", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          examId,
          remainingTime: Math.max(0, remaining),
          answersSnapshot: { selected: Array.from(selected), answers },
        }),
      });
    } catch (err) {
      message.error(err instanceof Error ? err.message : "暂停失败");
    }
  };

  const resumeExam = async () => {
    if (!examId) return;
    try {
      await apiRequest("/api/exam/resume", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ examId }),
      });
      setPaused(false);
    } catch (err) {
      message.error(err instanceof Error ? err.message : "继续失败");
    }
  };

  const finishExam = async () => {
    if (!examId || report) return;
    if (selectedCount < CHOOSE_COUNT) {
      void Modal.confirm({
        title: "确认提交",
        content: `你只选了 ${selectedCount} 道题，要求选做 ${CHOOSE_COUNT} 道。确定提交吗？`,
        onOk: () => {
          void doFinish().catch(() => {});
        },
      });
      return;
    }
    void doFinish().catch(() => {});
  };

  const doFinish = async () => {
    if (!examId || report) return;
    setLoading(true);
    try {
      // sync latest answers to server snapshot
      await apiRequest("/api/exam/pause", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          examId,
          remainingTime: Math.max(0, remaining),
          answersSnapshot: { selected: Array.from(selected), answers },
        }),
      });
      const r = await apiRequest<CaseReport>("/api/exam/case/finish", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ examId }),
      });
      setReport(r);
      setStarted(false);
      setPaused(false);
      if (timerRef.current) {
        clearInterval(timerRef.current);
        timerRef.current = null;
      }
    } catch (err) {
      message.error(err instanceof Error ? err.message : "提交失败");
    } finally {
      setLoading(false);
    }
  };

  const handleBlur = (questionId: string) => {
    void submitAnswer(questionId).catch(() => {});
  };

  if (report) {
    return (
      <Card style={{ maxWidth: 960, margin: "24px auto" }}>
        <Flex vertical gap="large" style={{ width: "100%" }}>
          <Result
            status={report.passed ? "success" : "warning"}
            title={`案例分析模拟考成绩：${report.score} / ${report.maxTotalScore}`}
            subTitle={
              <Space>
                <Text>{report.passed ? "恭喜，已达到合格线" : "未达合格线，继续加油"}</Text>
                <Text type="secondary">合格线：{report.passLine} 分</Text>
              </Space>
            }
          />
          <Title level={5}>AI 评分维度</Title>
          <List
            dataSource={report.dimensions}
            renderItem={(d) => (
              <List.Item>
                <Card style={{ width: "100%" }}>
                  <Flex vertical gap="small" style={{ width: "100%" }}>
                    <Space>
                      <Text strong>{d.name}</Text>
                      <Tag color="blue">权重 {d.weight}%</Tag>
                      <Text type="success">
                        {d.score} / {d.maxScore}
                      </Text>
                    </Space>
                    <Progress
                      percent={Math.round((d.score / d.maxScore) * 100)}
                      size="small"
                      status={d.score >= d.maxScore * 0.6 ? "success" : "exception"}
                    />
                    <Text>{d.comment}</Text>
                  </Flex>
                </Card>
              </List.Item>
            )}
          />
          {report.overallComment && (
            <>
              <Title level={5}>总体评价</Title>
              <Paragraph>{report.overallComment}</Paragraph>
            </>
          )}
          {report.improvementSuggestions.length > 0 && (
            <>
              <Title level={5}>改进建议</Title>
              <List
                dataSource={report.improvementSuggestions}
                renderItem={(s) => (
                  <List.Item>
                    <Text>• {s}</Text>
                  </List.Item>
                )}
              />
            </>
          )}
          <Button type="primary" onClick={() => navigate("/exam")}>
            返回模拟考首页
          </Button>
        </Flex>
      </Card>
    );
  }

  if (!paper || !started) {
    return (
      <Card style={{ maxWidth: 720, margin: "24px auto" }}>
        <Flex vertical style={{ width: "100%" }} align="center" gap="small">
          <Title level={4}>案例分析模拟考</Title>
          <Text>正在加载试卷...</Text>
        </Flex>
      </Card>
    );
  }

  return (
    <Card style={{ maxWidth: 960, margin: "24px auto" }}>
      <Flex vertical gap="large" style={{ width: "100%" }}>
        <Space style={{ justifyContent: "space-between", width: "100%" }}>
          <Title level={4} style={{ margin: 0 }}>
            案例分析模拟考
          </Title>
          <Space>
            <Tag color={remaining < 600 ? "error" : "blue"}>
              <Statistic
                value={formatSeconds(remaining)}
                title="剩余时间"
                styles={{
                  content: {
                    fontSize: 16,
                    fontWeight: 600,
                    color: token.colorText,
                  },
                }}
                formatter={() => formatSeconds(remaining)}
              />
            </Tag>
            {paused ? (
              <Button type="primary" onClick={resumeExam}>
                继续
              </Button>
            ) : (
              <Button onClick={pauseExam}>暂停</Button>
            )}
            <Button type="primary" danger onClick={finishExam} loading={loading}>
              提交
            </Button>
          </Space>
        </Space>

        <Card>
          <Space>
            <Text>
              请从 <Text strong>{TOTAL_QUESTIONS}</Text> 道题中选做{" "}
              <Text strong>{CHOOSE_COUNT}</Text> 道
            </Text>
            <Text>
              已选 <Text strong>{selectedCount}</Text> / {CHOOSE_COUNT}
            </Text>
            <Text>
              已作答 <Text strong>{answeredCount}</Text> / {CHOOSE_COUNT}
            </Text>
          </Space>
        </Card>

        <List
          dataSource={paper.questions}
          renderItem={(q, idx) => {
            const isSelected = selected.has(q.id);
            const ans = answers[q.id] ?? { answer: "" };
            return (
              <List.Item>
                <Card
                  style={{
                    width: "100%",
                    borderColor: isSelected ? token.colorPrimary : undefined,
                  }}
                  title={
                    <Space>
                      <Checkbox checked={isSelected} onChange={() => toggleQuestion(q.id)}>
                        第 {idx + 1} 题
                      </Checkbox>
                      <Tag>{q.chapter}</Tag>
                      <Tag
                        color={
                          q.difficulty === "hard"
                            ? "error"
                            : q.difficulty === "medium"
                              ? "warning"
                              : "success"
                        }
                      >
                        {q.difficulty}
                      </Tag>
                    </Space>
                  }
                >
                  <Flex vertical gap="small" style={{ width: "100%" }}>
                    <Paragraph>{q.question}</Paragraph>
                    {isSelected && (
                      <>
                        <Text strong>答题区</Text>
                        <TextArea
                          value={ans.answer}
                          onChange={(e) => updateAnswer(q.id, "answer", e.target.value)}
                          onBlur={() => handleBlur(q.id)}
                          rows={8}
                          placeholder="在此输入案例分析答案..."
                          disabled={paused}
                        />
                        <Collapse
                          items={[
                            {
                              key: "mermaid",
                              label: "Mermaid 图（可选）",
                              children: (
                                <TextArea
                                  value={ans.mermaid ?? ""}
                                  onChange={(e) => updateAnswer(q.id, "mermaid", e.target.value)}
                                  onBlur={() => handleBlur(q.id)}
                                  rows={4}
                                  placeholder="在此输入 Mermaid 语法描述架构图..."
                                  disabled={paused}
                                />
                              ),
                            },
                          ]}
                        />
                      </>
                    )}
                  </Flex>
                </Card>
              </List.Item>
            );
          }}
        />

        <Space style={{ justifyContent: "center", width: "100%" }}>
          <Button onClick={pauseExam} disabled={paused}>
            暂停
          </Button>
          <Button type="primary" onClick={resumeExam} disabled={!paused}>
            继续
          </Button>
          <Button type="primary" danger onClick={finishExam} loading={loading}>
            提交
          </Button>
        </Space>
      </Flex>
    </Card>
  );
}
